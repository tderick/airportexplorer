...
def compute_reviews_and_rating(airport: str):
    ...
    try:
        ...
        print(
            "Reviews and ratings computed for",
            airport,
            best_reviews,
            worst_reviews,
            average_rating,
    ...
    pipeline = [
        {"$match": {"airport": airport_code}},  # Match reviews for the specific airport
        {
            "$group": {
                ...
                "average_terminal_cleanliness_rating": {
                    "$avg": "$terminal_cleanliness_rating"
                },
                ...
                "average_wifi_connectivity_rating": {
                    "$avg": "$wifi_connectivity_rating"
                },
...
def countries_ident():
    pipeline = [
        ...
        },  # Unwind the airports array within each region
        {
            "$match": {
                "regions.airports.fivebestreviews": {
                    "$exists": False
                },  # Filter airports without the "fivebestreviews" array
                "regions.airports.fiveworstreview": {
                    "$exists": False
                },  # Filter airports without the "fiveworstreview" array
            }
        },
        {
            "$project": {"_id": 0, "ident": "$regions.airports.ident"}
        },  # Project the airport ident field
    ]
